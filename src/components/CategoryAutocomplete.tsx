      ...
      <div className="relative">
        ...
        {isOpen && (
          <div className="absolute z-50 w-full mt-2 bg-bg-card border border-border-ui rounded-xl shadow-2xl max-h-52 overflow-y-auto divide-y divide-border-ui/50 animate-fade-in transition-colors duration-200">
            {filtered.length === 0 ? (
              <div className="p-3 text-xs text-text-muted italic text-center">No categories found</div>
            ) : (
              filtered.map((cat) => (
                <button
                  key={cat.id}
                ...
                >
                  <span>{cat.name}</span>
                  <span className="text-[10px] bg-bg-input px-2 py-0.5 rounded text-text-muted">Add</span>
                </button>
              ))
            )}
          </div>
