...
interface MostUsedPunchlinesCardProps {
  ...
  onPunchlineClick: (punchline: { id: string; text: string }) => void;
}

export function MostUsedPunchlinesCard({ mostUsedPunchlines, onPunchlineClick }: MostUsedPunchlinesCardProps) {
  const intl = useIntl();
      ...
      {mostUsedPunchlines.length === 0 ? (
        ...
        <div className="divide-y divide-border-ui/60">
          {mostUsedPunchlines.map((p, idx) => (
            <div
              key={p.id}
              onClick={() => onPunchlineClick({ id: p.id, text: p.originalText })}
              className="py-4 flex items-center justify-between gap-4 first:pt-0 last:pb-0 cursor-pointer hover:bg-bg-input/40 px-3 -mx-3 rounded-xl transition-all duration-200"
            >
