...
import { FolderKanban, Tag, Activity, Users, LogOut } from "lucide-react";
...
export default function Navigation() {
  ...
  return (
    <>
        ...
        <div className="max-w-6xl mx-auto px-4 py-3 flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-[10px] md:text-xs text-slate-400 hidden md:block xl:hidden">
          ...
          <div className="flex items-center gap-3">
            ...
            {user && (
              <div className="flex items-center gap-2 border-l border-slate-800/80 pl-3">
                <span className="text-xs text-slate-400 hidden sm:block font-medium truncate max-w-[150px]" title={user.email}>
                  {user.email}
                </span>
                <button
                  onClick={signOut}
                  className="bg-slate-900 hover:bg-slate-800 hover:text-red-400 text-slate-400 p-2 rounded-lg border border-slate-800 transition-all cursor-pointer"
      ...
      <nav className="xl:hidden fixed bottom-0 left-0 right-0 bg-slate-950/90 backdrop-blur-xl border-t border-slate-850 py-2 pb-[calc(env(safe-area-inset-bottom)+0.5rem)] px-6 flex justify-around items-center z-50 shadow-2xl">
        {navItems.map((item) => {
          const Icon = item.icon;
          const isActive = pathname.startsWith(item.href);
          return (
